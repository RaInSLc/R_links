...
export function SettingsView(props: SettingsViewProps) {
  ...
  const deleteCache = async (entry: PackageCacheEntry) => {
    setCacheBusy(true);
    try {
      await invoke("delete_package_cache_entry", {
        package: entry.packageName, source: entry.source, version: entry.version,
        repository: entry.repository, realName: entry.realName,
      });
      setCacheEntries((current) => current.filter((item) => item !== entry));
    } catch (error) { window.alert(`缓存删除失败：${String(error)}`); } finally { setCacheBusy(false); }
  };
  const clearInvalidated = async () => {
    setCacheBusy(true);
    try { await invoke("clear_invalidated_cache"); await loadCache(); }
    catch (error) { window.alert(`清理失效缓存失败：${String(error)}`); } finally { setCacheBusy(false); }
  };
  const exportCache = async () => {
    ...
    try {
      ...
      anchor.href = url; anchor.download = `rlinks_cache_${new Date().toISOString().slice(0, 10)}.json`;
      document.body.appendChild(anchor); anchor.click(); document.body.removeChild(anchor);
      URL.revokeObjectURL(url);
    } catch (error) { window.alert(`缓存导出失败：${String(error)}`); } finally { setCacheBusy(false); }
  };
