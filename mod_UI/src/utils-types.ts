...
export interface SearchResult { package: string; requestedVersion: string; latestVersion: string; repository: string; realName: string; source: string; found: boolean; message: string; status?: string; stage?: string; verifiedCount?: number; upVotes?: number; downVotes?: number; invalidated?: boolean; }
export interface DependencyNode { package: string; source: string; version: string; depth: number; rootPackages: string[]; directDependencyCount: number; heavyDependencyCount: number; status: string; }
...
export interface PublicSettings { proxy: string; githubTokenConfigured: boolean; cranMirror: string; rLibPath: string; fullSearch: boolean; searchConcurrency: number; archiveGithubMajorGap: number; conditional: boolean; installDependencies: boolean; showRemoteVersion: boolean; useCache: boolean; maxCacheEntries: number; useFilter: boolean; resolveDependencies: boolean; maxDependencyDepth: number; includeLightDependencies: boolean; maxDependencyNodes: number; pinnedMethods: Method[]; pipIndex: string; condaChannels: string[]; }
export interface HistoryRecord { id: string; command: string; packageName: string; version: string; toolName: string; createdAt: string; input?: string; method?: Method; conditional?: boolean; installDependencies?: boolean; showRemoteVersion?: boolean; verifyInstall?: boolean; cranMirror?: string; }
...
export interface SmartSuggestion { id: string; title: string; detail: string; actionLabel?: string; action?: "method" | "enableVerify" | "replaceInput" | "openSettings" | "enableFullSearch" | "retrySearch"; method?: string; value?: string; }
...
export interface SearchPlanPreview { total: number; cranLike: number; repositories: number; archiveUrls: number; estimatedRequests: number; level: "idle" | "light" | "medium" | "heavy"; recommendedMode: string; summary: string; advice: string; }
