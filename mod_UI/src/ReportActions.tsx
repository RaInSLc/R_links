...
const copy = async (
  ...
  if (!content.trim()) { onStatusChange("安装命令尚未生成，请稍后重试"); return; }
  try { await writeText(content); onStatusChange(success); }
  catch (error) {
    onStatusChange(`复制失败: ${error instanceof Error ? error.message : String(error)}`);
  }
};
const download = (
  name: string,
...
export function ReportActions({
  ...
  const selected = results.filter((result) => selectedResults.has(selectionKey(result)));
  const exportCsv = () => {
    ...
    const rows = results.map((r) => [
      r.package, r.requestedVersion, r.latestVersion,
      sourceNames[r.source] ?? r.source,
      sourceCredibility(r.source, r.stage),
      r.repository, r.found ? "已验证" : r.status || "未找到",
      r.found ? getInstallCommand(r) : "",
    ].map(escape).join(","));
    download(
      "r_package_results.csv",
      `包名,请求版本,实际版本,来源,可信度,仓库,状态,安装命令\r\n${rows.join("\r\n")}`,
      "text/csv;charset=utf-8",
      (message) => onStatusChange(`${message}（${results.length} 条结果）`),
    );
  };
  const exportDeclaration = (kind: "renv" | "description") => {
    const content = kind === "renv"
      ? JSON.stringify({
        R: { Version: "4.4.0", Repositories: [{ Name: "CRAN", URL: "https://cloud.r-project.org" }] },
        Packages: Object.fromEntries(found.map((r) => [
          r.package,
          {
            Package: r.package,
            Version: r.latestVersion || "unknown",
            Source: r.source,
    ...
    download(
      kind === "renv" ? "renv.lock" : "DESCRIPTION",
      content,
  ...
  const exportMarkdown = () => {
    const now = new Date();
    const status = (r: SearchResult) =>
      r.found ? "已验证"
        : r.status === "timeout" ? "超时"
        : r.status === "rateLimited" ? "频率限制"
        : r.status === "error" ? "检索异常"
        : "未找到";
    const lines = [
      `# R 包检索报告`, ``,
      ...
      `> 总计: ${results.length} | 已验证: ${found.length}`, ``,
      `| 包名 | 版本 | 来源 | 可信度 | 仓库 | 状态 |`,
      `|---|---|---|---|---|---|`,
      ...results.map((r) =>
        `| ${r.package} | ${r.latestVersion || "-"} | ${sourceNames[r.source] ?? r.source} | ${sourceCredibility(r.source, r.stage)} | ${r.repository || "-"} | ${status(r)} |`,
      ),
    ];
    download(
      `r-packages-report-${now.toISOString().slice(0, 10)}.md`,
      lines.join("\n"),
  ...
  const openPages = async () => {
    const pageResults = [...new Map(
      found
        .filter((r) => ["cran", "bioc", "github", "r-forge"].includes(r.source))
        .map((r) => [`${r.source}:${r.package}`, r]),
    ).values()];
    if (pageResults.length > 5 && !window.confirm(`将要打开 ${pageResults.length} 个浏览器页面，是否继续？`))
      return;
    let opened = 0;
    for (const result of pageResults) {
      ...
      } catch { /* continue opening remaining pages */ }
    }
    onStatusChange(`已打开 ${opened} 个来源网页`);
  };
  const script = () => {
    const commands = [...new Set(found.map(getInstallCommand))];
    const lines = [
  ...
  const summary = () => {
    const groups = ["已验证", "未找到", "异常"];
    const values = [
      uniquePackages(found),
      uniquePackages(results.filter(isPlainMissingResult)),
      uniquePackages(results.filter(isErrorResult)),
    ];
    void copy(
      [
