          ...
          {sorted.map((record, idx) => {
            ...
            return (
              <Fragment key={record.id}>
                ...
                <article
                  className={`history-item ${selectedIds.has(record.id) ? "selected" : ""} ${histNavIndex === idx ? "nav-selected" : ""}`}
                  onMouseEnter={() => setHistNavIndex(idx)}
                  onDoubleClick={() => onApplyRecord(record)}
                  title="双击应用此记录"
