...
export function ReportResultsTable({ results, searching, resultFilter, setResultFilter, selectedResults, setSelectedResults, onStatusChange, onCancelPackage, onRetryMissing }: Props) {
  ...
  const handleCopy = async (result: SearchResult, key: string) => { try { await writeText(getInstallCommand(result)); setCopiedKey(key); onStatusChange(result.found ? `已复制 ${result.package} 的安装指令` : `已复制包名 ${result.package}`); window.setTimeout(() => setCopiedKey(null), 1500); } catch (error) { onStatusChange(`复制安装指令失败: ${error instanceof Error ? error.message : String(error)}`); } };
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { const tag = (event.target as HTMLElement)?.tagName; if (tag === "INPUT" || tag === "TEXTAREA" || sortedResults.length === 0) return; if (event.key === "ArrowDown") { event.preventDefault(); setSelectedRowIndex((index) => Math.min(index + 1, sortedResults.length - 1)); } else if (event.key === "ArrowUp") { event.preventDefault(); setSelectedRowIndex((index) => Math.max(index - 1, 0)); } else if (event.key === "Enter" && selectedRowIndex >= 0) { void handleCopy(sortedResults[selectedRowIndex], `${sortedResults[selectedRowIndex].package}-kbd`); } else if (event.key === " " && selectedRowIndex >= 0) { event.preventDefault(); toggleExpanded(`${sortedResults[selectedRowIndex].package}-${sortedResults[selectedRowIndex].source}-${selectedRowIndex}`); } };
