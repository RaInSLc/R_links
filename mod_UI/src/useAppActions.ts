...
export function useAppActions(context: AppActionContext) {
  ...
  useEffect(() => { if (view !== "workspace") return; const onKeydown = (e: KeyboardEvent) => { if (!(e.ctrlKey || e.metaKey)) return; if (e.key === "Enter") { e.preventDefault(); if (searching) stopSearch(); else if (input.trim() && !inputTooLarge) handleStartSearch(); } else if (e.shiftKey && e.key.toLowerCase() === "c") { e.preventDefault(); void copyScript(); } else if (!e.shiftKey && e.key.toLowerCase() === "s") { e.preventDefault(); downloadScript(); } else if (e.shiftKey && e.key.toLowerCase() === "k") { e.preventDefault(); if (!searching && input.trim()) acceptInputValue("", "manual"); } else if (!e.shiftKey && e.key.toLowerCase() === "d") { e.preventDefault(); if (!searching && input.trim()) acceptInputValue(dedupePackageInput(input, inputRules), "manual"); } }; window.addEventListener("keydown", onKeydown); return () => window.removeEventListener("keydown", onKeydown); });
  return {
    acceptInputValue,
    pasteInput,
    handleStartSearch,
