...
pub struct SearchState {
    pub(crate) inner: Mutex<SearchStateInner>,
}
...
pub(crate) struct SearchRunGuard<'a> {
    state: &'a SearchState,
    run_id: u64,
...
impl SearchState {
    ...
    pub(crate) fn request_stop(&self, run_id: u64) -> bool {
    ...
    pub(crate) fn set_paused(&self, run_id: u64, paused: bool) -> bool {
        let mut inner = self.lock_inner();
        if !inner.running || inner.run_id != run_id {
            return false;
        }
    ...
    pub(crate) fn cancel_package(&self, run_id: u64, package: &str) -> bool {
        ...
        inner
            .cancelled_packages
            .insert(package.to_ascii_lowercase());
        true
    }
    ...
    #[cfg(test)]
    pub(crate) fn run_id_for_test(&self) -> u64 {
