...
fn read_limited_to_string_rejects_symbolic_links() {
    ...
    let directory = std::env::temp_dir().join(format!("mod-ui-read-link-{}", unique_file_suffix()));
...
fn saturated_backup_directory_reuses_overflow_file() {
    ...
    backup_corrupt_path(&directory, "settings.json", "first").expect("饱和目录应可写入固定备份");
...
fn rejects_oversized_history_save_fields() {
    ...
    let error = validate_history_save_payload(&history).expect_err("超长历史命令应在清洗前被拒绝");
...
fn rejects_history_save_fields_with_control_characters() {
    ...
    let error = validate_history_save_payload(&history).expect_err("控制字符字段应在清洗前被拒绝");
...
fn rejects_history_save_payload_total_bytes() {
    ...
    let error =
        validate_history_save_payload(&history).expect_err("总大小过大的历史保存 payload 应被拒绝");
